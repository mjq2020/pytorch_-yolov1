...
class Loss(nn.Module):
    ...
    def __init__(self, device='cpu',feature_size=7, num_bboxes=2, num_classes=20, lambda_coord=5.0, lambda_noobj=0.5):
        """ Constructor.
        Args:
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        noobj_conf_mask = torch.BoolTensor(noobj_pred.size()).fill_(0)  # [n_noobj, N]
        for b in range(B):
            noobj_conf_mask[:, 4 + b * 5] = 1  # 没有目标置信度置1，noobj_conf_mask[:, 4] = 1; noobj_conf_mask[:, 9] = 1

        noobj_pred_conf = noobj_pred[noobj_conf_mask]  # [n_noobj x 2=len([conf1, conf2])]
        ...
        coord_response_mask = torch.BoolTensor(bbox_target.size()).fill_(0)  # [n_coord x B, 5]
        coord_not_response_mask = torch.BoolTensor(bbox_target.size()).fill_(1)  # [n_coord x B, 5]
        bbox_target_iou = torch.zeros(bbox_target.size())  # [n_coord x B, 5], only the last 1=(conf,) is used
        # return F.mse_loss(bbox_pred,bbox_target)
        ...
        for i in range(0, bbox_target.size(0), B):
            ...
            pred_xyxy[:, 2:4] = torch.sqrt(pred[:, :2] / float(S) + 0.5 * pred[:, 2:4])
        ...
        bbox_pred_response = bbox_pred[coord_response_mask].view(-1, 5).to(self.device)  # [n_response, 5]
        bbox_target_response = bbox_target[coord_response_mask].view(-1,5).to(self.device)  # [n_response, 5], only the first 4=(x, y, w, h) are used
        target_iou = bbox_target_iou[coord_response_mask].view(-1,5).to(self.device) # [n_response, 5], only the last 1=(conf,) is used
